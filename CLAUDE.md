...
- **Migration** (`migrateFromWorldview`, idempotent via the `migratedFromWorldview` flag): one-time import of the former worldview module **A3** answers — `household`/`family_key` tuples → people+`me→person` edges; `friends_key` names → `friend` edges; `pets` → person nodes with `attrs.about=kind` and a `pet` edge. Reads by the old field keys, so it works after A3 was removed from the registry. Called at `app.js` `handleStart()` and `worldview-ui.open()` after both models load.
- **Registry change** (`app/data/worldview-questions.json`): **module A3 deleted**; its one genuinely-flat field `address_terms` ("what do you call people") moved into **A1 About You**. People/pets are now the graph, not questionnaire fields — so `worldview.buildBlock()` no longer emits them and there's no double-injection.
...
- CACHE_VERSION/APP_VERSION → `0.2.27`; `relationships.js` added to the sw precache. **Verified in preview:** model CRUD + private-withholding `buildBlock` + idempotent migration of mock A3 data; the People editor add→display→mark-private→badge flow; registry shows no A3 and `address_terms` under A1; no console errors. *On-disk persistence to `relationships.json` needs a granted FSA folder — confirm on the tablet.* **Scope held:** small typed people+edges with a free `attrs` bag — no graph DB, no third-party-relations editor yet.
