...
**About Me content width (v0.2.20).** The cards were full-viewport-width (after Ken's earlier "use all the space" ask removed the cap) — too wide on a real tablet. Now `#worldviewContent` is a **readable centred column** (`max-width: 40rem; margin: 0 auto`) and the side-keyboard reservation moved from `#worldviewContent` padding to **`#worldviewScreen` padding**, so both the header and the column clear the keyboard on **either** side and the column centres in the remaining region. Verified both sides clear the keyboard (incl. the keyboard-on-left case Ken flagged). 40rem is tunable.
